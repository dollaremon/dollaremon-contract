...
*/
contract DollaremonBonusAggregator is Ownable, IBonusAggregator{
    using SafeMath for uint256;
 ...
 */
contract DollaremonBEP20 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
...
// MasterDollaremon is the master of RADS and sRADS.
// The Ownership of this contract is going to be transferred to a timelock
contract MasterDollaremon is Ownable, IMasterBonus {
    using SafeMath for uint256;
    using SafeBEP20 for DollaremonBEP20;
    ...
    DollaremonBonusAggregator public bonusAggregator;
    // The RADS TOKEN!
    DollaremonBEP20 public rads;
    // The SRADS TOKEN!
    DollaremonBEP20 public sRads;
    // Dev address.
    ...
    uint256 public minimumEmissionRate = 100 finney;
    ...
    constructor(
        DollaremonBEP20 _rads,
        DollaremonBEP20 _srads,
        DollaremonBonusAggregator _bonusAggregator,
        address _devaddr,
        address _feeAddress,
    ...
    // Allow DollaremonBonusAggregator to add bonus on a single pool by id to a specific user
    ...
    // Deposit LP tokens to MasterDollaremon for RADS allocation.
    ...
    // Withdraw LP tokens from MasterDollaremon.
    ...
    function safeRadsTransfer(address _to, uint256 _total) internal {

        (uint256 _amount, uint256 _fee) = userFee(_total);
        if( _fee > 0 ){
            rads.safeTransfer(feeAddress, _fee);
        }
    ...
    function safeSRadsTransfer(address _to, uint256 _total) internal {

        (uint256 _amount, uint256 _fee) = userFee(_total);
        if( _fee > 0 ){
            sRads.safeTransfer(feeAddress, _fee);
        }
    ...
    function userFee(uint256 _total) private view returns (uint256 _amount, uint256 _amount_fee ) {
        ...
        if( harvestFeeBP > 0 ){
            _amount_fee = _total.mul(harvestFeeBP).div(10000);
    ...
    function setHarvestFee(uint256 _harvestFeeBP) external onlyOwner {
        require( _harvestFeeBP <= 1000, "can't be more than 10%" );
